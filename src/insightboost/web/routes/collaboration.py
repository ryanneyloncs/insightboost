...
_active_cursors: dict[str, dict[str, dict[str, Any]]] = {}  # session_id -> user_id -> cursor
...
def get_session(session_id: str):
    ...
    if expires_at < datetime.utcnow() and session["status"] == SessionStatus.ACTIVE.value:
...
def get_comments(session_id: str):
    ...
    if visualization_id:
        comments = [c for c in comments if c.get("visualization_id") == visualization_id]
...
@collaboration_bp.route("/sessions/<session_id>/comments/<comment_id>", methods=["DELETE"])
...
def create_snapshot(session_id: str):
    ...
    user_id = data.get("user_id")
    name = data.get("name", f"Snapshot {len(_session_snapshots.get(session_id, [])) + 1}")
...
@collaboration_bp.route("/sessions/<session_id>/snapshots/<snapshot_id>", methods=["GET"])
...
@collaboration_bp.route("/sessions/<session_id>/snapshots/<snapshot_id>", methods=["DELETE"])
