...
class DataAnalyzer:
    ...
    def analyze_column(
        ...
        is_potential_id = (
            unique_pct > 95
            and data_type in (ColumnDataType.INTEGER, ColumnDataType.CATEGORICAL)
            and (
                "id" in column.lower()
        ...
        is_potential_date = data_type == ColumnDataType.DATETIME or (
            ...
            and any(
                kw in column.lower() for kw in ["date", "time", "created", "updated"]
            )
        )
    ...
    def analyze_distribution(
        ...
        outlier_percentage = (
            (outlier_count / len(clean_series) * 100) if len(clean_series) > 0 else 0
        )
    ...
    def identify_quality_issues(
        ...
        if missing["missing_percentage"] > 20:
            issues.append(
                f"High missing value rate: {missing['missing_percentage']:.1f}%"
            )

        for col, info in missing["column_missing"].items():
            if info["percentage"] > 50:
                issues.append(
                    f"Column '{col}' has {info['percentage']:.1f}% missing values"
                )
        ...
        if dup_count > 0:
            issues.append(
                f"Found {dup_count} duplicate rows ({dup_count / len(df) * 100:.1f}%)"
            )
        ...
        for col in df.select_dtypes(include=["object"]).columns:
            ...
            if unique_ratio > 0.9:
                issues.append(
                    f"Column '{col}' has very high cardinality (possible ID column)"
                )
